...
4. **Deduplication**: Customizations are automatically deduplicated by canonical resolved file path (`Path.resolve()`) so a rule or skill is never injected twice in a single turn. Downstream repositories that want both `AGENTS.md` and `GEMINI.md` with single-injection path deduplication can symlink `ln -s AGENTS.md GEMINI.md`.
